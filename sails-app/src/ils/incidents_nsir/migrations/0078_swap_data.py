...
from incidents_nsir.models import RadiationTreatmentTechnique
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Assign all pre-existing single radiation treatment techniques (FK) to the newly
        # added M2M field
        for inc in Incident.objects.all():
            if inc.radiation_treatment_technique != None:
                inc.radiation_treatment_techniques.add(inc.radiation_treatment_technique)
                inc.save()

    def backwards(self, orm):
        # Assign the technique with largest pk (i.e. highest priority... SBRT over IMRT)
        # from the M2M field to the singular technique (FK)
        for inc in Incident.objects.all():
            if inc.radiation_treatment_techniques.count() > 0:
                inc.radiation_treatment_technique = inc.radiation_treatment_techniques.latest('id')
                inc.save()
